...
echo "Starting persistent smoke script...\n";
...
try {
    ...
    echo "Persisting simulation (committing DB changes).\n";
    DB::commit();

    // Print resulting pivot and component values for inspection
    echo "\nResulting pivot entries for order id={$order->id}:\n";
    foreach ($order->components as $c) {
        echo "- {$c->name} (pivot qty_used={$c->pivot->quantity_used})\n";
    }

    echo "\nSample component values after commit:\n";
    $sample = Component::whereIn('id', array_keys($componentsToAttach))->take(10)->get();
    foreach ($sample as $s) {
        echo "- {$s->name} (avail={$s->quantity_available}, stok_used={$s->stok_used})\n";
    }

    echo "Persistent smoke script finished.\n";
} catch (\Exception $e) {
    DB::rollBack();
    echo "Error during persistent simulation: " . $e->getMessage() . "\n";
}
